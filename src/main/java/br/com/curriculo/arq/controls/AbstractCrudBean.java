...
import br.com.curriculo.arq.exception.ServiceDataAccessException;
import br.com.curriculo.arq.persistence.PageData;
import br.com.curriculo.arq.persistence.Persistent;
import br.com.curriculo.arq.service.CrudService;
 ...
 * (serviço) e crias as páginas de cadastro e consulta. Feito isto, todo o
 * mecanismo de busca, inserção, edição e exclusão já estará automaticamente
 * disponível.
 *
...
public abstract class AbstractCrudBean<P extends Persistent, S extends CrudService<P>> extends AbstractBean {
	...
	private LazyDataModel<P> dataLazyModel;
	...
	private boolean updateRowCount = false;
	...
	private void initializeDataLazyModel() {
		if (log.isDebugEnabled()) {
			log.debug("initializeDataLazyModel invocado " + this);
		}
		dataLazyModel = new LazyDataModel<P>() {
			private static final long serialVersionUID = 1L;
			...
			@Override
			public List<P> load(int first, int pageSize, String sortField, SortOrder sortOrder,
					Map<String, FilterMeta> filters) {
				if (log.isDebugEnabled()) {
					log.debug("Carregando os registros de {0} de {1} à {2}",
							new Object[] { entityToSearch.getClass().getSimpleName(), first, (first + pageSize) });
				}
				searchExecuted = true;
				boolean internalSort = (sortOrder == sortOrder.ASCENDING ? true : false);
				pageData = loadSearchData(first, pageSize, sortField, internalSort);
			...
			public int getRowCount() {
				if (log.isDebugEnabled()) {
					log.debug(String.format(
							"getRowCount() invocado em %s. (total=%s, searchExecuted=%s, updateRowCount=%s)", this,
							total, searchExecuted, updateRowCount));
				}
				if (total == null || searchExecuted || updateRowCount) {
					if (log.isDebugEnabled()) {
						log.debug("Invocando loadSearchDataRowCount()");
					}
					searchExecuted = false;
					updateRowCount = false;

					if (pageData != null) {
						total = pageData.getCountAll();
					} else {
						total = 0;
					}
				}
			...
			@Override
			public P getRowData(String rowKey) {
				for (P ent : dataLazyModel) {
					if (ent.getId().equals(rowKey)) {
						return ent;
					}
				}
				return null;
			...
			@Override
			public Object getRowKey(P ent) {
				return ent.getId();
			}

		};

		// Valor padrão para evitar o erro "Divid by zero".
		// Mesmo este valor sendo setado aqui, para evitar o referido erro,
		// o valor preponderante é mesmo definido no atributo "rows"
		// do componente "<p:dataTable>".
		dataLazyModel.setPageSize(10);
	...
	public LazyDataModel<P> getDataLazyModel() {
		return dataLazyModel;
	}

	public void setDataLazyModel(LazyDataModel<P> dataLazyModel) {
		this.dataLazyModel = dataLazyModel;
	}
	...
	public void search() {
		try {
			...
			/*
			 * Atenção: Por detalhes da implementação do LazyDataModel do PrimeFaces, estes
			 * métodos não são invocados diretamente aqui. Devido a uma inversão de
			 * controle, eles são invocados pelo próprio componente LazyDataModel toda vez
			 * que o componente visual associado a ele, no caso o p:dataTable, é
			 * renderizado.
			 */
			// loadSearchData...
			...
			/*
			 * Atenção: Por detalhes da implementação do LazyDataModel do PrimeFaces, este
			 * método "hook" está sendo invocado no método setWrappedData, que foi
			 * sobreecrito, do próprio componente LazyDataModel.
			 */
			// onAfterSearch();
	...
	// Método temporariamente habilitado por conta de problemas
	// ainda não resolvidos ao passar a instância "objRow" do
	// componente "p:dataTable" como parâmetro do método, assim
	// como foi feito no método de edição.
	@Deprecated
	public void delete() {
		delete(this.entity);
	}
	...
	public void save(P entityToSave, Action actionAfterSave) {
		...
		try {
			...
			if (action == Action.INSERT) {
				...
				addInfo("Registro inserido com sucesso");
			} else if (action == Action.EDIT) {
				executeServiceUpdate(entityToSave);
				addInfo("Registro editado com sucesso");
			}
			/*
